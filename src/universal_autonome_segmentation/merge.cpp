...
namespace MERGE{
  DATA::graph mergeRegions(DATA::graph &g){
    ...
    for(int i = 0; i < g.N; i++){
      ...
      res.mean_vector[components[i]][0] += g.previous_mean_vector[i][0] * g.pixel_count[i];
      res.mean_vector[components[i]][1] += g.previous_mean_vector[i][1] * g.pixel_count[i];
      res.mean_vector[components[i]][2] += g.previous_mean_vector[i][2] * g.pixel_count[i];
